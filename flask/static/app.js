...
(function () {
  ...
  async function loadDir(path) {
    ...
    let html = `<div class="browser-cwd">${escapeHtml(t("flask.browser.cwd", { path: data.cwd, n: data.csv_count }))}</div>`;
    ...
    if (data.csv_files && data.csv_files.length > 0) {
      html += `<div class="browser-section">${escapeHtml(t("flask.browser.filesSection"))}</div>`;
  ...
  function runAnalysis(path, bbox) {
    ...
    els.status.textContent = bbox ? t("flask.status.connectArea") : t("flask.status.connect");
    ...
    src.addEventListener("start", (e) => {
      ...
      const scope = d.bbox ? t("flask.status.scopeArea") : "";
      els.status.textContent = t("flask.status.folder", { folder: d.folder, total: d.total, scope });
    });
    ...
    src.addEventListener("progress", (e) => {
      ...
      els.status.textContent = t("flask.status.processed", { done: d.done, total: d.total });
    });
    src.addEventListener("error", (e) => {
      try {
        const d = JSON.parse(e.data || "{}");
        if (d.message) console.warn("Parse error:", d);
      } catch (_) { /* reconnect */ }
    });
  ...
  function startAnalysis() {
    ...
    if (!path) {
      els.status.textContent = t("flask.status.pickFirst");
      return;
    }
    pendingPath = path;
    showModal();
  ...
  els.stop.addEventListener("click", () => {
    ...
    els.stop.disabled = true;
    els.progress.hidden = true;
    els.status.textContent = t("flask.status.stopped");
  });
  ...
  function renderBallistics(res) {
    ...
    L.circleMarker(land, {
      ...
      `<b>${escapeHtml(t("ballistics.result.predicted"))}</b><br>${land[0].toFixed(6)}, ${land[1].toFixed(6)}<br>` +
    ...
    els.bResult.innerHTML = `
      ...
      <div><a href="${gmaps}" target="_blank">${escapeHtml(t("ballistics.result.openMaps"))}</a></div>
    `;
  }
  ...
  const nav = { watchId: null, myPos: null, myMarker: null, accuracyCircle: null,
                target: null, targetMarker: null, deviceHeading: null,
                pickingTarget: false, deviceOrientationListening: false };
  ...
  function formatDistance(m) {
    return m < 1000 ? `${m.toFixed(0)} ${t("compass.m")}` : `${(m/1000).toFixed(m<10000?2:1)} ${t("compass.km")}`;
  }

  function updateCompass() {
    if (!nav.myPos || !nav.target) { els.compass.hidden = true; return; }
    ...
    if (els.compassArrow) els.compassArrow.style.transform = `translate(-50%, -100%) rotate(${brg - heading}deg)`;
    if (els.compassRing) els.compassRing.style.transform = `rotate(${-heading}deg)`;
  ...
  function updateMyPosition(pos) {
    const lat = pos.coords.latitude, lon = pos.coords.longitude, acc = pos.coords.accuracy || 30;
    const first = !nav.myPos;
    nav.myPos = { lat, lon, accuracy: acc };
    const popupTxt = `<b>${escapeHtml(t("nav.popup.myLocation"))}</b><br>${lat.toFixed(6)}, ${lon.toFixed(6)}<br>±${acc.toFixed(0)} m`;
    if (!nav.myMarker) {
      nav.myMarker = L.marker([lat, lon], {
        icon: L.divIcon({ html: userIconHtml(), className: "user-loc-icon", iconSize: [20,20] }),
      }).addTo(navLayer).bindPopup(popupTxt);
      nav.accuracyCircle = L.circle([lat, lon], { radius: acc, color: "#4363d8", fillColor: "#4363d8", fillOpacity: 0.12, weight: 1 }).addTo(navLayer);
    } else {
      nav.myMarker.setLatLng([lat, lon]).setPopupContent(popupTxt);
      nav.accuracyCircle.setLatLng([lat, lon]).setRadius(acc);
    }
    if (first) map.setView([lat, lon], Math.max(map.getZoom(), 15));
    updateCompass();
  }
  ...
  function startLocation() {
    if (!navigator.geolocation) { els.status.textContent = t("nav.gpsUnavailable"); return; }
    nav.watchId = navigator.geolocation.watchPosition(updateMyPosition, onGeolocationError,
  ...
  els.locateBtn.addEventListener("click", () => {
    if (nav.watchId != null) stopLocation(); else startLocation();
  });
  ...
  window.addEventListener("fpv-langchange", () => {
    els.locateBtn.textContent = nav.watchId != null ? t("nav.locateStop") : t("nav.locate");
    els.pickTargetBtn.textContent = nav.pickingTarget ? t("nav.pickTargetActive") : t("nav.pickTarget");
